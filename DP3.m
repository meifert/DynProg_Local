global LadungAnfang Pe Verbrauch Charge Mv DPErgebnispfad T0 DZ Zeit Revs
clear L PeOpt R Charge Mv Pe
        ...
        if Clutch(k)>0.99                          % Kupplung starr
            if Mb(k)==0                               % kein Bremsmoment
                ...
                for n=1:length(X)                     % Schleife über Zustandsindex
                    for Pe=0:USchritt*DZ(k)/9.55:Umax % Schleife über elektische Leistung
                        Mv=Mp(k);
                        LadungAnfang=X(n);
                        sim('Fahrzeug')
